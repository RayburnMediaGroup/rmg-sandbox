...
import { loadProfile, saveProfile, type ProfileData } from "@/lib/bandProfile";
...
export default function BandPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (ref && !localStorage.getItem("bs_ref")) {
        ...
        // Increment referral visit count for the referring artist
  ...
  return (
    <main style={{ background: tokens.bg, minHeight: "100vh", color: tokens.text, ...T }}>
      ...
      {/* ── Hero header — not sticky, scrolls away ── */}
      <div style={{ borderBottom: `1px solid ${tokens.border}` }}>
        ...
        {/* Identity row — overlaps cover photo bottom */}
          ...
          <div style={{ maxWidth: 860, margin: "0 auto", display: "flex", flexDirection: isMobile ? "column" : "row", alignItems: isMobile ? "flex-start" : "flex-end", gap: isMobile ? "0.75rem" : "1.75rem" }}>
            ...
            <div style={{ flex: 1, minWidth: 0, paddingBottom: 4 }}>
              ...
              <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", marginBottom: "0.6rem" }}>
                ...
                {!isMobile && <Link href="/band/stage-plot" style={{ border: `1px solid ${tokens.border2}`, color: tokens.muted, ...T, fontSize: "0.68rem", letterSpacing: "0.06em", textTransform: "uppercase", textDecoration: "none", padding: "7px 14px", borderRadius: 4 }}>Stage Plot</Link>}
              </div>
              {/* Platform chips */}
              <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>
                {(profile.links ?? artist.allLinks ?? []).filter(l => ["Spotify","Apple Music","YouTube","Amazon Music"].includes(l.label)).map(l => {
      ...
      {editMode && (
        <div style={{ background: tokens.accent + "18", borderBottom: `1px solid ${tokens.accent}44`, padding: "7px 40px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <button onClick={() => setEditMode(false)} style={{ ...lbl, background: "transparent", border: `1px solid ${tokens.accent}55`, borderRadius: 4, color: tokens.accent, padding: "3px 10px", cursor: "pointer" }}>Done</button>
        </div>
      )}

      {/* ── Active section ── */}
      {active === "about"   && <AboutSection   profile={profile} tokens={tokens} isArtist={editMode} onUpdate={onUpdate} />}
      {active === "music"   && <MusicSection   profile={profile} tokens={tokens} />}
      {active === "lyrics"  && <LyricsSection  profile={profile} tokens={tokens} />}
      ...
      {active === "timeline" && <TimelineSection profile={profile} tokens={tokens} />}
      {active === "press"   && <PressSection   profile={profile} tokens={tokens} isArtist={editMode} onUpdate={onUpdate} />}
      {active === "stats"   && <StatsSection   profile={profile} tokens={tokens} />}
      {active === "merch"   && <MerchSection   profile={profile} tokens={tokens} />}
      ...
      {active === "contact"  && <ContactSection  profile={profile} tokens={tokens} isArtist={editMode} onUpdate={onUpdate} />}
      ...
      {showDashboard && artistUnlocked && (
        <ArtistDashboard
          ...
          onLock={() => { setArtistUnlocked(false); setEditMode(false); }}
        />
      )}
