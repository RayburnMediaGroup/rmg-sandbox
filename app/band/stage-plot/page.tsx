...
export default function StagePlotPage() {
  ...
  const [genre, setGenre] = useState(artist.genre[0] ?? "");
  ...
  return (
    <main style={{ background: tokens.bg, minHeight: "100vh", color: tokens.text, ...T }}>
      ...
      <div style={{ padding: "16px 32px", borderBottom: `1px solid ${tokens.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <Link href="/band" style={{ ...lbl, color: tokens.muted, textDecoration: "none" }}>← {artist.name}</Link>
        <span style={{ ...lbl, color: acc }}>Stage Plot & Tech Rider</span>
        <Link href="/band/stage-plot/edit" style={{ ...lbl, color: tokens.muted, textDecoration: "none" }}>Edit →</Link>
      </div>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: isMobile ? "24px 16px 48px" : "32px 32px 64px" }}>
        ...
        <div style={{ marginBottom: "2.5rem" }}>
          <h1 style={{ ...T, fontWeight: 700, fontSize: "1.6rem", color: tokens.text, margin: "0 0 0.3rem" }}>{artist.name}</h1>
          <p style={{ ...body, color: tokens.muted, fontSize: "0.75rem" }}>
            {artist.genre.join(", ")} · {artist.origin} · {artist.members.length}-piece band
          </p>
          <div style={{ display: "flex", gap: "1rem", marginTop: "0.75rem", flexWrap: "wrap" }}>
            <a href={`mailto:${artist.bookingEmail}`} style={{ ...lbl, color: acc, textDecoration: "none" }}>Booking: {artist.bookingEmail}</a>
            {artist.bookingContact && <span style={{ ...lbl, color: tokens.muted2 }}>{artist.bookingContact}</span>}
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2.5rem" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
            ...
            <div>
              ...
              <p style={{ ...body, fontSize: "0.78rem", color: tokens.muted }}>{artist.bookingContact}</p>
              <a href={`mailto:${artist.bookingEmail}`} style={{ ...body, fontSize: "0.78rem", color: acc, textDecoration: "none" }}>{artist.bookingEmail}</a>
            </div>
