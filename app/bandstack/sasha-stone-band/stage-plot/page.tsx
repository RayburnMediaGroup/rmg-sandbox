...
const PROFILE_KEY = "bandstack-sasha-stone-band-v1";
...
export default function SashaStoneStagePlotPage() {
  const isMobile = useMobile();
  const [colorMode, setColorMode] = useState<"dark"|"light">("dark");
  const [genre,     setGenre]     = useState("");
  const [plot,      setPlot]      = useState<PlotData | null>(null);

  // Profile fields — read from localStorage, not hardcoded
  const [bandName,        setBandName]        = useState("Sasha Stone Band");
  const [origin,          setOrigin]          = useState("Fort Collins, CO");
  ...
  const inputs = (plot?.inputs?.filter(r => r.instrument) ?? []).length > 0
    ...
    : [
      ...
      { ch: 10, instrument: "Acoustic DI",   mic: "DI Box" },
      { ch: 11, instrument: "Lead Vox",      mic: "Shure SM58 / Beta 58A" },
      { ch: 12, instrument: "BGV",           mic: "Shure SM58" },
    ];
  ...
  const monitors = (plot?.monitors?.filter(m => m.position) ?? []).length > 0
    ...
    : [
      ...
      { mix: 4, position: "Vocals (wedge)",      notes: "Both vocals, acoustic" },
    ];
  ...
  return (
    <main style={{ background: tokens.bg, minHeight: "100vh", color: tokens.text, ...T }}>
      ...
      <div style={{ padding: "16px 32px", borderBottom: `1px solid ${tokens.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <Link href="/bandstack/sasha-stone-band" style={{ ...lbl, color: tokens.muted, textDecoration: "none" }}>← {bandName}</Link>
        <span style={{ ...lbl, color: acc }}>Stage Plot & Tech Rider</span>
        <Link href="/bandstack/sasha-stone-band/stage-plot/edit" style={{ ...lbl, color: tokens.muted, textDecoration: "none" }}>Edit →</Link>
      </div>
